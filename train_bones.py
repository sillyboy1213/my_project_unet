...
# 训练日志
writer = SummaryWriter(os.path.join(root_path, 'log/bones/train', model_name + '_{}fold'.format(fold) + str(int(time.time()))))
val_writer = SummaryWriter(os.path.join(os.path.join(root_path, 'log/bones/val', model_name) + '_{}fold'.format(fold) + str(int(time.time()))))

# 训练集路径
# train_path = os.path.join(root_path, 'media/Datasets/bladder/Augdata_5folds', 'train{}'.format(fold), 'npy')
train_path = os.path.join(root_path, 'media/Datasets/Bones/raw_data')
val_path = os.path.join(root_path, 'media/Datasets/Bones/raw_data')


def main():
    # 定义网络
    net = Baseline(num_classes=Bones.num_classes, depth=depth).to('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # 训练集加载
    train_set = Bones.Dataset(train_path, 'train', fold, joint_transform=None, center_crop=center_crop,
                                    transform=input_transform, target_transform=target_transform)
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=6)
    # 验证集加载
    val_set = Bones.Dataset(val_path, 'val', fold,
    ...
    # 定义损失函数
    if loss_name == 'dice':
        criterion = SoftDiceLoss(Bones.num_classes).to('cuda' if torch.cuda.is_available() else 'cpu')
...
def train(train_loader, val_loader, net, criterion, optimizer, scheduler, warm_scheduler, early_stopping, num_epoches,
    ...
    for epoch in range(1, num_epoches + 1):
        st = time.time()
        train_class_dices = np.array([0] * (Bones.num_classes - 1), dtype=np.float64)
        val_class_dices = np.array([0] * (Bones.num_classes - 1), dtype=np.float64)
        val_dice_arr = []
        train_losses = []
        val_losses = []
        ...
        for batch, ((input, mask), file_name) in enumerate(train_loader, 1):
            ...
            plt.close()
            class_dice = []
            for i in range(1, Bones.num_classes):
